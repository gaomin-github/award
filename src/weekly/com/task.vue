  ...
  <div class="task-wrapper">
    ...
    <header class="user">
      ...
      <div class="user-score">
        <circle-chart
          :score="totalValue ? score / totalValue : 0"
        ></circle-chart>
      </div>
    </header>
    <nav>
  ...
  methods: {
    ...
    back() {
      let schedules = this.scheduleList.filter((schedule, index) => {
        if (schedule.content && schedule.content.length > 0) return true;
        else return false;
      });
      this.initTask({
        taskId: this.taskId,
    ...
    _initTask() {
      request.get(`/task/task?userId=${this.curUser.userId}`).then((res) => {
        if (res.status === 200 && res.data) {
          this.initTask(res.data[0]);
        } else {
          request
            .get(`/task/createTask?userId=${this.curUser.userId}`)
            .then((newRes) => {
