<template>
    <div class="assess-wrapper">
        <div class="editor" v-if="isEditing">
            ...
            <div>
                ...
                <div class="editor-img" v-if="imgs && imgs.length > 0">
                    <div v-for="imgItem in imgs" :key="imgItem.id" class="editor-img-item">
                        <assess-img :data="imgItem.data" @chooseImg="chooseImg"></assess-img>
                    </div>
        ...
        <div class="assess-con" v-else>
            ...
            <div class="assess-in">
                <input placeholder="添加评价" type="text" @click="handleEditing(true)" />
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        delAssess(item) {
            request
                .get(`/task/assess?type=delete&areaId=${areaId}`)
                .then(res => {
                    if (res.status === 200) {
                        return res.data;
                        this.$emit("updateArea");
