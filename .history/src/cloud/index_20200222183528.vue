<template>
  <section class="cloud-wrapper">
    ...
    <ui-modal v-model="showResult"></ui-modal>
  </section>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    _getAward() {
      ...
      result.map((item, key) => {
        this._storeResult(item);
      });
      this.resultList = this.curResultList;
    },
    _storeResult(newResults) {
      let awardStorage = localStorage.get("award");
      let results = [];
      if (awardStorage) {
        results.push(JSON.parse(awardStorage));
      }
      newResults.map(item => {
        results[`${item.phone}_${item.name}`] = this.curBtnType;
      });
      localStorage.set(`${item.phone}_${item.name}`, this.curBtnType);
    },
    _getResult(item) {
      let result = localStorage.get("award");
      if (result) {
        let resultList = JSON.parse(result);
        return resultList[`${item.phone}_${item.name}`];
      }
      return null;
    },
    getAllResult() {},
